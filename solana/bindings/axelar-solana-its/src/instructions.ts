...
export class ItsInstructions {
    ...
    readonly tokenManagerInstructions: TokenManagerInstructions;

    constructor(
        itsId: PublicKey, gatewayRootPda: PublicKey, provider: AnchorProvider,
    ) {
        ...
        this.interchainToken = new InterchainTokenInstructions(itsId, gatewayRootPda, provider);
        this.tokenManagerInstructions = new TokenManagerInstructions(itsId, gatewayRootPda, provider);
    }
    ...
    setTrustedChain(params: {
        ...
        return this.program.methods
            .setTrustedChain(params.chainName)
            .accounts({
                payer: params.payer,
                programDataAddress: this.programDataAddress,
                gatewayRootPda: this.gatewayRootPda,
    ...
    approveDeployRemoteInterchainToken(params: {
        ...
        const tokenId = interchainTokenId(params.deployer, Buffer.from(params.salt));
        ...
        const [deployApprovalPda] = findDeploymentApprovalPda(params.payer, tokenId, params.destinationChain);
        return this.program.methods.approveDeployRemoteInterchainToken(params.deployer, params.salt, params.destinationChain, Buffer.from(params.destinationMinter)).accounts({
            payer: params.payer,
            tokenManagerPda,
            rolesPda,
            deployApprovalPda,
    ...
    revokeDeployRemoteInterchainToken(params: {
        ...
        const tokenId = interchainTokenId(params.deployer, Buffer.from(params.salt));
        const [deployApprovalPda] = findDeploymentApprovalPda(params.payer, tokenId, params.destinationChain);
        return this.program.methods.revokeDeployRemoteInterchainToken(params.deployer, params.salt, params.destinationChain).accounts({
            payer: params.payer,
            deployApprovalPda,
            systemProgram: SystemProgram.programId,
        });
    ...
    registerCanonicalInterchainToken(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, tokenId);
        const tokenManagerAta = getAssociatedTokenAddressSync(params.mint, tokenManagerPda, true, params.tokenProgram, ASSOCIATED_TOKEN_PROGRAM_ID);
    ...
    deployRemoteCanonicalInterchainToken(params: {
        ...
        const [callContractSigningPda, signingPdaBump] = findCallContractSigningPda();
        const [tokenMetadataAccount] = findMetadataPda(params.mint);

        return this.program.methods.deployRemoteCanonicalInterchainToken(
    ...
    deployInterchainToken(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, tokenId);
        const [interchainTokenPda] = findInterchainTokenPda(this.itsRootPda, tokenId);
        ...
        const [itsUserRolesPda] = findUserRolesPda(tokenManagerPda, this.itsRootPda);
        ...
        return this.program.methods
            .deployInterchainToken(params.salt, params.name, params.symbol, params.decimals)
            .accounts({
                payer: params.payer,
                gatewayRootPda: this.gatewayRootPda,
                systemProgram: SystemProgram.programId,
    ...
    deployRemoteInterchainToken(params: {
        ...
        return this.program.methods.deployRemoteInterchainToken(
            params.salt,
            params.destinationChain,
            params.gasValue,
            signingPdaBump,
    ...
    deployRemoteInterchainTokenWithMinter(params: {
        ...
        const [callContractSigningPda, signingPdaBump] = findCallContractSigningPda();
        const [tokenMetadataAccount] = findMetadataPda(interchainTokenPda);
        const [deployApprovalPda] = findDeploymentApprovalPda(params.minter, tokenId, params.destinationChain);
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, tokenId);
        const [minterRolesPda] = findUserRolesPda(tokenManagerPda, params.minter);

        return this.program.methods.deployRemoteInterchainTokenWithMinter(
            params.salt,
            params.destinationChain,
    ...
    registerTokenMetadata(params: {
        ...
        return this.program.methods.registerTokenMetadata(params.gasValue, signingPdaBump).accounts({
            payer: params.payer,
            mint: params.mint,
            tokenProgram: params.tokenProgram,
            gatewayRootPda: this.gatewayRootPda,
    ...
    registerCustomToken(params: {
        ...
        const [tokenMetadataAccount] = findMetadataPda(params.mint);
        const [itsUserRolesPda] = findUserRolesPda(tokenManagerPda, this.itsRootPda);
        const operatorRolesPda = params.operator ? findUserRolesPda(tokenManagerPda, params.operator)[0] : null;

        return this.program.methods.registerCustomToken(
            params.salt,
    ...
    linkToken(params: {
        ...
        return this.program.methods.linkToken(
            params.salt,
            params.destinationChain,
    ...
    interchainTransfer(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        ...
        return this.program.methods.interchainTransfer(
            params.tokenId,
            params.destinationChain,
    ...
    callContractWithInterchainToken(params: {
        ...
        return this.program.methods.callContractWithInterchainToken(
            params.tokenId,
            params.destinationChain,
            Buffer.from(params.destinationAddress),
            params.amount,
    ...
    callContractWithInterchainTokenOffchainData(params: {
        ...
        gasService: PublicKey;
        gasConfigPda: PublicKey;
    }): [ReturnType<Program['methods']['callContractWithInterchainToken']>, Uint8Array] {
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        ...
        const transferPayload = utils.defaultAbiCoder.encode(
            ...
            [MESSAGE_TYPE_INTERCHAIN_TRANSFER, params.tokenId, utils.arrayify(params.sourceAccount.toBytes()), params.destinationAddress, params.amount, params.data],
        );
        ...
        return [this.program.methods.callContractWithInterchainTokenOffchainData(
            params.tokenId,
            params.destinationChain,
            Buffer.from(params.destinationAddress),
            params.amount,
    ...
    setFlowLimit(params: {
        ...
        const [tokenManagerUserRolesPda] = findUserRolesPda(tokenManagerPda, this.itsRootPda);
    ...
    transferOperatorship(params: {
        ...
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(this.itsRootPda, params.newOperator);
        const [payerRolesPda] = findUserRolesPda(this.itsRootPda, params.payer);

        return this.program.methods.operatorTransferOperatorship({
            roles: { operator: {} },
    ...
    proposeOperatorship(params: {
        ...
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(this.itsRootPda, params.newOperator);
        const [payerRolesPda] = findUserRolesPda(this.itsRootPda, params.payer);
        const [proposalPda, proposalPdaBump] = findProposalPda(this.itsRootPda, params.payer, params.newOperator);

        return this.program.methods.operatorProposeOperatorship({
            roles: { operator: {} },
            destinationRolesPdaBump,
            proposalPdaBump,
        }).accounts({
    ...
    acceptOperatorship(params: {
        ...
        const [payerRolesPda, payerRolesPdaBump] = findUserRolesPda(this.itsRootPda, params.payer);
        const [oldOperatorRolesPda] = findUserRolesPda(this.itsRootPda, params.oldOperator);
        const [proposalPda, proposalPdaBump] = findProposalPda(this.itsRootPda, params.oldOperator, params.payer);

        return this.program.methods.operatorAcceptOperatorship({
            roles: { operator: {} },
...
export class InterchainTokenInstructions {
    ...
    mint(params: {
        ...
        return this.program.methods.interchainTokenMint(
            params.amount
        ).accounts({
            mint: params.mint,
    ...
    transferMintership(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.newMinter);
        const [payerRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);

        return this.program.methods.interchainTokenTransferMintership({
            roles: { minter: {} },
            destinationRolesPdaBump,
            proposalPdaBump: null
        }).accounts({
    ...
    proposeMintership(params: {
        ...
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.newMinter);
        const [payerRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);
        const [proposalPda, proposalPdaBump] = findProposalPda(tokenManagerPda, params.payer, params.newMinter);

        return this.program.methods.interchainTokenProposeMintership({
            roles: { minter: {} },
            destinationRolesPdaBump,
            proposalPdaBump,
        }).accounts({
    ...
    acceptMintership(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        const [payerRolesPda, payerRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.payer);
        const [oldMinterRolesPda] = findUserRolesPda(tokenManagerPda, params.oldMinter);
        const [proposalPda, proposalPdaBump] = findProposalPda(tokenManagerPda, params.oldMinter, params.payer);

        return this.program.methods.interchainTokenAcceptMintership({
            roles: { minter: {} },
            destinationRolesPdaBump: payerRolesPdaBump,
            proposalPdaBump,
        }).accounts({
...
export class TokenManagerInstructions {
    ...
    handOverMintAuthority(params: {
        ...
        return this.program.methods.tokenManagerHandOverMintAuthority(params.tokenId).accounts({
            payer: params.payer,
            mint: params.mint,
            gatewayRootPda: this.gatewayRootPda,
            itsRootPda: this.itsRootPda,
    ...
    setFlowLimit(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        const [tokenManagerUserRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);
        const [itsUserRolesPda] = findUserRolesPda(this.itsRootPda, params.payer);

        return this.program.methods.tokenManagerSetFlowLimit(params.flowLimit
        ).accounts({
            payer: params.payer,
            itsRootPda: this.itsRootPda,
            tokenManagerPda,
    ...
    addFlowLimiter(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.flowLimiter);
        const [payerRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);

        return this.program.methods.tokenManagerAddFlowLimiter({
            roles: { flowLimiter: {} },
            destinationRolesPdaBump,
            proposalPdaBump: null,
        }).accounts({
    ...
    removeFlowLimiter(params: {
        ...
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.flowLimiter);
        const [payerRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);
        return this.program.methods.tokenManagerRemoveFlowLimiter({
            roles: { flowLimiter: {} },
            destinationRolesPdaBump,
            proposalPdaBump: null,
        }).accounts({
    ...
    transferOperatorship(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.newOperator);
        const [payerRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);

        return this.program.methods.tokenManagerTransferOperatorship({
            roles: { operator: {} },
            destinationRolesPdaBump,
            proposalPdaBump: null
        }).accounts({
    ...
    proposeOperatorship(params: {
        ...
        const [destinationUserRolesPda, destinationRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.newOperator);
        const [payerRolesPda] = findUserRolesPda(tokenManagerPda, params.payer);
        const [proposalPda, proposalPdaBump] = findProposalPda(tokenManagerPda, params.payer, params.newOperator);

        return this.program.methods.tokenManagerProposeOperatorship({
            roles: { operator: {} },
            destinationRolesPdaBump,
            proposalPdaBump,
        }).accounts({
    ...
    acceptOperatorship(params: {
        ...
        const [tokenManagerPda] = findTokenManagerPda(this.itsRootPda, Buffer.from(params.tokenId));
        const [payerRolesPda, payerRolesPdaBump] = findUserRolesPda(tokenManagerPda, params.payer);
        const [oldOperatorRolesPda] = findUserRolesPda(tokenManagerPda, params.oldOperator);
        const [proposalPda, proposalPdaBump] = findProposalPda(tokenManagerPda, params.oldOperator, params.payer);

        return this.program.methods.tokenManagerAcceptOperatorship({
            roles: { operator: {} },
            destinationRolesPdaBump: payerRolesPdaBump,
            proposalPdaBump,
        }).accounts({
