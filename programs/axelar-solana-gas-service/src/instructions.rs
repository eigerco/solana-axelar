...
pub enum GasServiceInstruction {
    ...
    /// 1. `[]` The `gas_collector` account of this PDA.
...
pub enum PayWithSplToken {
    ...
    /// Collect fees that have accrued in SPL tokens (gas_collector only).
    CollectFees {
    ...
    /// Refund previously collected SPL token fees (gas_collector only).
    Refund {
        /// A 64-byte unique transaction identifier
        tx_hash: [u8; 64],
        /// The index of the log entry in the transaction
...
pub enum PayWithNativeToken {
    ...
    /// Collect accrued native SOL fees (gas_collector only).
    ///
    /// Accounts expected:
    /// 1. `[signer, read-only]` The `gas_collector` account authorized to collect fees.
    ...
    /// Refund previously collected native SOL fees (gas_collector only).
    ///
    /// Accounts expected:
    /// 1. `[signer, read-only]` The `gas_collector` account authorized to issue refunds.
...
pub fn init_config(
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(*gas_collector, true),
...
/// Builds an instruction for the gas_collector to collect native SOL fees.
///
/// # Errors
/// - ix data cannot be serialized
...
/// Builds an instruction for the gas_collector to refund previously collected native SOL fees.
///
/// # Errors
/// - ix data cannot be serialized
...
/// Builds an instruction for the gas_collector to collect SPL fees.
///
/// # Errors
/// - ix data cannot be serialized
#[allow(clippy::too_many_arguments)]
...
/// Builds an instruction for the gas_collector to refund previously collected SPL fees.
///
/// # Errors
/// - ix data cannot be serialized
#[allow(clippy::too_many_arguments)]
